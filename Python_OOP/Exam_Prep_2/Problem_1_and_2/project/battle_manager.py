...
class BattleManager:
    ...
    def add_zone(self, zone_type: str, zone_code: str):
        ...
        searched = [z for z in self.zones if z.code == zone_code]
        if searched:
            raise Exception("Zone already exists!")

        result = self.VALID_ZONES[zone_type](zone_code)
        self.zones.append(result)
    ...
    def add_battleship(self, ship_type: str, name: str, health: int, hit_strength: int):
        ...
        result = self.VALID_SHIPS[ship_type](name, health, hit_strength)
        self.ships.append(result)
    ...
    def remove_battleship(self, ship_name: str):
        try:
            searched = [s for s in self.ships if s.name == ship_name][0]
        except IndexError:
            return "No ship with this name!"

        if not searched.is_available:
            return f"The ship participates in zone battles! Removal is impossible!"

        self.ships.remove(searched)
        return f"Successfully removed ship {searched.name}."
