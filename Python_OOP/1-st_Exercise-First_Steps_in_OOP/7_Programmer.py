class Programmer():
    ...
    def watch_course(self, course_name: str, current_langauge: str, skills_earned: int):
        if current_langauge == self.language:
            self.skills += skills_earned
            return f"{self.name} watched {course_name}"
        return f"{self.name} does not know {current_langauge}"
